package com.stash.plugins

import com.stash.StashProperties

private val stashProperties = StashProperties(project)
...
val patterns: List<String> = (
    listOf(
        ...
        "**/src/**/*.kts",
        "!*/build/*",
    ) + stashProperties.ktlintAdditionalPatterns.orEmpty()
    )
    .sortedDescending()
...
dependencies {
    ktlint(stashProperties.libraries.ktlint) {
        attributes {
            attribute(Bundling.BUNDLING_ATTRIBUTE, objects.named(Bundling.EXTERNAL))
        }
    }

    ktlint(stashProperties.libraries.stashKtlintRules)
    ktlintConfig(stashProperties.libraries.stashKtlintConfig)
}
...
tasks.register("ktlint", JavaExec::class) {
    ...
    args = listOf(
        "--reporter=plain",
        "--reporter=checkstyle,output=${layout.buildDirectory}/reports/ktlint/ktlint.xml",
        "--color",
